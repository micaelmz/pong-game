...
class Scenario:
    ...
    def create_score_board(self):
        ...
        self.score_board.goto(x=-35, y=250)
    ...
    def show_scored_line(self, position):
        self.scored_line.goto(x=position, y=0)
        self.scored_line.showturtle()

    def show_score_board(self, paddle_left_score, paddle_right_score):
        self.score_board.clear()
        self.score_board.write(f"{paddle_left_score}   {paddle_right_score}", font=FONT)

    def waiting_message(self):
        self.waiting_text.hideturtle()
        self.waiting_text.color('white')
        self.waiting_text.penup()
        self.waiting_text.goto(x=-220, y=100)
        self.waiting_text.clear()
        self.waiting_text.write(f"Press SPACE to roll the ball.", font=FONT)
